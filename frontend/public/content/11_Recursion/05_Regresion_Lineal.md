...
![Gauss](/content/11_Recursion/gauss.jpg)
...
```
![ej0_scatter](/content/11_Recursion/ej0_scatter.png)
...
![ej0_posiblesrectas](/content/11_Recursion/ej0_posiblesrectas.png)
...
![eq_suma_de_cuadrados](/content/11_Recursion/eq_suma_de_cuadrados.png)
...
Recordá que en la [Sección 5.3](/programa/units/aleatoriedad/numpy#fórmulas-matemáticas) vimos que calcular el promedio de estos errores cuadráticos es muy sencillo en numpy. También podés usar la función `mean_squared_error` del módulo `sklearn.metrics` que trae muchas métricas muy útiles.
...
![ej0_ajuste](/content/11_Recursion/ej0_ajuste.png)
...
plt.show()
```
![ejsint_scatter](/content/11_Recursion/ejsint_scatter.png)
...
![ejsint_ajuste](/content/11_Recursion/ejsint_ajuste.png)
...
plt.show()
```
![ejcuad_scatter](/content/11_Recursion/ejcuad_scatter.png)
...
![ejcuad_ajustelineal](/content/11_Recursion/ejcuad_ajustelineal.png)
...
![ejcuad_ajusteconx2](/content/11_Recursion/ejcuad_ajusteconx2.png)
...
Al igual que el modelo de clustering que usamos en el [Ejercicio 9.19](/programa/units/clases-y-objetos/teledeteccion#ejercicio-919-clasificacion-automatica) de teledetección, el objeto de tipo `LinearRegression` de `sklearn.liearmodel` también tiene un método `fit()` que permite ajustar el modelo a los datos y otro `predict()` que permite usar el modelo ajustado con nuevos datos.

Acá rehacemos el primer ejemplo que dimos ([Sección 11.5](/programa/units/recursion/regresion-lineal#ejemplo-el-modelo-de-cuadrados-mínimos)), usando pandas y el módulo `linear_model`.
...
![ejsk1_ajuste](/content/11_Recursion/ejsk1_ajuste.png)
...
-->
![eq_reg_lin_multiple](/content/11_Recursion/eq_reg_lin_multiple.png)
...
![AIC](/content/11_Recursion/AIC.png)
