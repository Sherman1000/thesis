...
En el [Ejercicio 3.16](/programa/units/datos/formato#ejercicio-316-un-desafio-de-formato) (o el [Ejercicio 3.15](/programa/units/datos/formato#ejercicio-315-agregar-encabezados)) escribiste un programa llamado `tabla_informe.py` que imprime un informe con el balance de compra y venta de frutas en un camión.
...
Copiá el contenido de tu programa `tabla_informe.py` en un nuevo archivo llamado `informe_funciones.py` y guardá este nuevo archivo en el directorio de ejercicios de la clase actual. Por si no tenés una versión andando del archivo `tabla_informe.py`, te dejamos [acá](/content/06_Organizacion_y_Complejidad/tabla_informe.py) una para que puedas usar.
