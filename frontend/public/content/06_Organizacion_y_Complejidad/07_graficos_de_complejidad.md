...
A partir del código de búsqueda binaria (`busqueda_binaria(lista, x)`) introducido en la [Sección 6.5](/programa/units/organizacion-y-complejidad/busqueda-binaria#búsqueda-binaria), definí una función `busqueda_binaria_comps(lista, x)` que devuelva la posición del elemento en la lista y la cantidad de comparaciones que realizó el algoritmo para encontrarlo o decidir que no está.
...
![Imagen Complejidad Secuencial](/content/06_Organizacion_y_Complejidad/compl_sec.png)
